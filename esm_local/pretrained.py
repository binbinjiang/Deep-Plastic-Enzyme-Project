...
from . import pretrained, MSATransformer, Alphabet, BatchConverter, FastaBatchedDataset, ESM2, ProteinBertModel
...
# bink
def load_esm1b_esm2_woPrePara(esm_seq_type):
    esmtype2modelname = {"esm2":"esm2_t33_650M_UR50D","esm1b":"esm1b_t33_650M_UR50S"}
    assert esm_seq_type in esmtype2modelname.keys()
    model_name = esmtype2modelname[esm_seq_type]

    model_data, regression_data = _download_model_and_regression_data(model_name)
    # load_model_and_alphabet_core(model_name, model_data, regression_data)
...
# bink
def load_esm1b_esm2_with_custom_PrePara(esm_seq_type, esm_encoder_layers):
    esmtype2modelname = {"esm2":"esm2_t33_650M_UR50D","esm1b":"esm1b_t33_650M_UR50S"}
    assert esm_seq_type in esmtype2modelname.keys()
    model_name = esmtype2modelname[esm_seq_type]
    ...
    load_state_dict_bool = True
    if model_name.startswith("esm2"):
        esm2_cfg = model_data["cfg"]["model"]
        # print(cfg.encoder_layers)# 33
        if esm_encoder_layers!=esm2_cfg.encoder_layers:
            load_state_dict_bool = False
            model_data["cfg"]["model"].encoder_layers = esm_encoder_layers

        model, alphabet, model_state = _load_model_and_alphabet_core_v2(model_data)
    else:
        # for k, v in vars(model_data["args"]).items():
        #     # print(k)
        #     if k=="encoder_layers":
        #         print(k,v) # encoder_layers 33
        esm1b_cfg = vars(model_data["args"])
        if esm_encoder_layers!=esm1b_cfg["encoder_layers"]:
            load_state_dict_bool = False
            esm1b_cfg["encoder_layers"] = esm_encoder_layers
    ...
    # print(regression_data is not None)
    if not load_state_dict_bool:
        allowed_missing = {"contact_head.regression.weight", "contact_head.regression.bias"}
